    ...
    for block in blocks:
        ...
        for seen in seen_entries:
            ...
            if title_match and author_match:
                ...
                if conflicts:
                    logger.error(f"Conflicting fields found while deduplicating exact match '{key}' & '{seen['key']}' (Title: {raw_title}):")
                    for k, v1, v2 in conflicts:
                        logger.error(f"  - Field '{k}': kept='{v1}' vs dropped='{v2}'")
                else:
                    logger.info(f"Deduplicated identical entry: '{key}' (merged into '{seen['key']}')")
                break
                
            elif title_match and not author_match:
                logger.error(f"Title match with differing authors:\n  1) [{seen['key']}] {seen['raw_author']}\n  2) [{key}] {raw_author}")
                
            elif key_match:
                logger.error(f"Duplicate key '{key}' found but refers to different entries! (Titles/Authors differ).")
