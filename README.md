...
4. Make sure that the entry is sorted and the key is standardized by running `make standardize` before committing.

# Automated Tools

To maintain the quality of the bibliography, this repository includes several automated Python tools. You can run the entire standardization pipeline using the provided `Makefile`:
...
```bash
make standardize OPTS="-vv"
```
- `-v` prints standard updates (e.g., which keys were changed or sanitized).
- `-vv` prints detailed warnings and errors (e.g., conflicting properties between duplicates, missing author fields, or unknown macros).
...
- **Standardized keys**. All the keys are standardized automatically using the pattern `[auth:lower][year][veryshorttitle:lower]`. The `make keys` tool enforces this pattern:
